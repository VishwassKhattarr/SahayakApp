...
import pkg from 'pg';
import { Resend } from 'resend'; // <-- UPDATED: Import Resend instead of Nodemailer
const { Pool } = pkg;
...
// --- Resend Setup ---
// <-- UPDATED: Initialize Resend client. The old transporter object is removed.
    ...
    try {
        ...
        if (GOOGLE_FORM_BASE_URL && EMAIL_FIELD_ID && WORKSHEET_ID_FIELD_ID && !GOOGLE_FORM_BASE_URL.includes('YOUR_FORM_ID') && studentEmails.length > 0) {
            ...
            for (const email of studentEmails) {
                ...
                // --- UPDATED: This block is changed to use Resend ---
                try {
                    ...
                    if (error) {
                        // Resend reports errors in this 'error' object
                        console.error(`Failed to send worksheet to ${email}:`, error);
                    } else {
                        // Success
                        emailsSentCount++;
                    }
                } catch (err) {
                    // This catches network/request-level exceptions
                    console.error(`Exception occurred while sending email to ${email}:`, err);
                }
