...
        const query = `
            ...
            JOIN subjects sub ON tca.subject_id = sub.id -- FIXED: Was tca.class_subject_id
...
            `SELECT 
                csj.id as class_subject_id, -- Get class_subject_id from the join
                tca.section_id, 
                cls.class_name, 
                sec.section_name, 
                sub.subject_name
...
            WHERE ch.class_subject_id = $2 -- This part was already correct
...
                Also provide a separate, detailed answer key. Format the output in Markdown with two sections: "# Worksheet: ${chapter_name}" and "# Answer Key".`;
            ...
            // Using a different model as "gemini-2.5-flash" might not be valid
            // Using "gemini-pro" as a stable, valid model name.
            // If "gemini-2.5-flash" is correct, you can change it back.
...
                 const parts = fullContent.split("# Answer key");
                worksheetContent = parts[0].replace(`# Worksheet: ${chapter_name}`, "").trim();
score:                answerKeyContent = parts[1].trim();
            }
...
export const getWorksheet = async (req, res) => {
    ...
    // This function was already correct as it doesn't use the
    // teacher_class_assignments table in a way that caused the error.
...
                ch.chapter_name
    _         FROM generated_worksheets gw
